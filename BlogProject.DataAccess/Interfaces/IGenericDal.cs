...
namespace BlogProject.DataAccess.Interfaces
{
    public interface IGenericDal<TEntity> where TEntity : class, ITable, new()
    {
        ...
        Task<List<TEntity>> GelAllSortAsync<Tkey>(Expression<Func<TEntity, Tkey>> keySelector);

        Task<List<TEntity>> GelAllSortAsync<Tkey>(Expression<Func<TEntity, bool>> filter,
